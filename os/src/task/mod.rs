...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.first_run_time_ms = timer::get_time_ms();
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].first_run_time_ms == 0 {
                inner.tasks[next].first_run_time_ms = timer::get_time_ms();
            }
    ...
    fn get_current_first_run_time_ms(&self) -> usize {
        let inner = self.inner.exclusive_access();
        inner.tasks[inner.current_task].first_run_time_ms
    }

    fn get_current_syscall_times(&self) -> [u32; config::MAX_SYSCALL_NUM] {
        let inner = self.inner.exclusive_access();
...
/// Get the first run time of current task
pub fn get_current_first_run_time_ms() -> usize {
    TASK_MANAGER.get_current_first_run_time_ms()
}

/// Get the syscall times of current task
pub fn get_current_syscall_times() -> [u32; config::MAX_SYSCALL_NUM] {
